...
exports.event_insert = function (req, res) {
    ...
    c.query(query.event_insert,[name,start,end,comment,location,company])
        ...
        .on('end', function () {
            c.query(query.lecture_material_insert,null)
                .on('result', function (res) {
                    res.on('row', function (row) {
                        sending.push(row);
                    });
...
//안내글 등록
exports.infoLetterInsert = function (req, res) {
    var sending = [];
    var eventId = req.body.eventIda;
    var imageFile = req.files.file[0];
    var name = imageFile.name;
    var path = imageFile.path;
    var MultiPath ='';

    var outputPath = __dirname + '/../../../event_solution/client/src/files/testFile/' + Date.now() + '_' + name;
    MultiPath = outputPath;
    MultiPath = MultiPath.substring(110);
    console.log(MultiPath);
    var ws = fs.createWriteStream(outputPath);
    fs.createReadStream(path).pipe(ws);

    c.query(query.infoLetterInsert,[MultiPath,eventId])
        .on('result', function (res) {
            res.on('row', function (row) {
...
//초대글 등록
exports.welcomeLetterInsert = function (req, res) {
    var sending = [];
    var eventId = req.body.eventIdb;
    var imageFile = req.files.file[0];
    var name = imageFile.name;
    var path = imageFile.path;
    var MultiPath ='';
    ...
    c.query(query.welcomeLetterInsert,[MultiPath,eventId])
        .on('result', function (res) {
            res.on('row', function (row) {
...
//일정표 등록
exports.agendaLetterInsert = function (req, res) {
    var sending = [];
    var eventId = req.body.eventIdc;
    var imageFile = req.files.file[0];
    var name = imageFile.name;
    var path = imageFile.path;
    var MultiPath ='';
    ...
    c.query(query.agendaLetterInsert,[MultiPath,eventId])
        .on('result', function (res) {
            res.on('row', function (row) {
...
exports.lectureInsert = function (req, res) {
    var sending = [];
    var eventId = req.body.eventIdd;
    ...
    c.query(query.lectureInsert,[eventId,eventId,lectureTitle,MultiPath])
        .on('result', function (res) {
            res.on('row', function (row) {
...
//주변안내 등록 tourismeInsertGo
exports.tourismeInsert = function (req, res) {
    var sending = [];
    var eventId = req.body.eventIde;
    var lectureTitle = req.body.info_title1;
    var info_comment1= req.body.info_comment1;
    ...
    c.query(query.tourismeInsert,[eventId,eventId,lectureTitle,info_comment1,MultiPath])
        .on('result', function (res) {
            res.on('row', function (row) {
